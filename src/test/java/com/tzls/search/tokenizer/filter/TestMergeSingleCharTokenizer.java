...
public class TestMergeSingleCharTokenizer {
	...
	public void test() throws IOException{
		...
		TokenStream ts = new MergerFilterFactory.MergeSingleCharFilter(tokenizer,true);
	...
	public void testEn() throws IOException{
		tokenizer.setReader(new StringReader("a上 d c天w"));
		TokenStream ts = new MergerFilterFactory.MergeSingleCharFilter(tokenizer,true);
		CharTermAttribute term = ts.getAttribute(CharTermAttribute.class);
		TypeAttribute type = ts.getAttribute(TypeAttribute.class);
		while(ts.incrementToken()){
